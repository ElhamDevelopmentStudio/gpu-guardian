...
3. **[x] FR-21 + FR-22 (Action/telemetry/decision logging + reports)**
  - Added `internal/report` with `SessionReport`, `ThermalProfile`, and `RecoveryMetrics`.
  - Added report generation from structured logs via `report.Generate(...)` with defaults for throughput floor.
  - Added CLI entrypoint `guardian report` with flags:
    - `--control-log`
    - `--telemetry-log`
